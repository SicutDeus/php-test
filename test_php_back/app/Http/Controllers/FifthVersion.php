...
class FifthVersion extends Controller
{
    ...
    public static $tableName_historyConfigs = ([
    ...
    public static function objectAndInnerRelationsCurrentCreated($table, $original_id)
    {
        ...
        foreach ($this_cfg['foreign_tables'] as $field => $foreign_table) {
            ...
            $inner_for_foreign_object = self::objectAndInnerRelationsCurrentCreated(
                $foreign_table['table'],
                $foreign_object->id
            );
    ...
    private static function getHistoryOrFirstCreatedObject(
        $tableName,
        $original_id,
        $is_first_created = 1,
        $start_date = null,
    ...
    private static function objectAndInnerRelationsHistory(
        $table,
        $original_id,
        $start_date,
        $end_date,
    ...
    public function test5($table, $original_id)
    {
        ...
        $nested = self::combineArrayByTime($nested);
        $nested[now()->format('Y-m-d H:i:s.u')][] = [$this_cfg['front_one_name'] => $current];
        $created_array = [
            date_create($created['created_at'])->format(self::$date_format) =>
    ...
    private static function combineArrayByTime($input)
    {
        $result_array = array();
